...
"$CONDA_BIN" run -n "$ENV_NAME" python -m pip install -U "regex>=2025.10.22" "transformers>=4.45.0" "peft>=0.13.0" "accelerate>=0.34.0" "datasets>=2.21.0" "trl>=0.10.0" "safetensors>=0.4.4"
"$CONDA_BIN" run -n "$ENV_NAME" python -m pip install --no-deps -U "git+https://github.com/hiyouga/LlamaFactory.git" --no-build-isolation
